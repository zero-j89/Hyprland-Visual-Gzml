...
wallpaper_output="$cache_dir/posterize_${focused_monitor}.png"
...
magick "$wallpaper_original" -posterize 4 "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Posterize effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Posterize applied"
