...
wallpaper_output="$cache_dir/zoomed_${focused_monitor}.png"
...
magick "$wallpaper_original" -gravity Center -extent 1:1 "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Zoomed effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Zoomed applied"
