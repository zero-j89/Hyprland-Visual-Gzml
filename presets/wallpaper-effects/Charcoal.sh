...
wallpaper_output="$cache_dir/charcoal_${focused_monitor}.png"
...
magick "$wallpaper_original" -charcoal 0x5 "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Charcoal effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Charcoal applied"
