...
wallpaper_output="$cache_dir/edge_detect_${focused_monitor}.png"
...
magick "$wallpaper_original" -edge 1 "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Edge Detect effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Edge Detect applied"
