...
wallpaper_output="$cache_dir/vignette_${focused_monitor}.png"
...
magick "$wallpaper_original" -vignette 0x3 "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Vignette effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Vignette applied"
