...
wallpaper_output="$cache_dir/black_and_white_${focused_monitor}.png"
...
magick "$wallpaper_original" -colorspace gray -sigmoidal-contrast 10,40% "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Black & White effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Black & White applied"
