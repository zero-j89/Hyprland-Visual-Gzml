...
wallpaper_output="$cache_dir/polaroid_${focused_monitor}.png"
...
magick "$wallpaper_original" -polaroid 0 "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Polaroid effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Polaroid applied"
