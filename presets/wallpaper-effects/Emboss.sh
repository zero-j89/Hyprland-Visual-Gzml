...
wallpaper_output="$cache_dir/emboss_${focused_monitor}.png"
...
magick "$wallpaper_original" -emboss 0x5 "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Emboss effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Emboss applied"
