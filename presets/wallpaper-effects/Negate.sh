...
wallpaper_output="$cache_dir/negate_${focused_monitor}.png"
...
magick "$wallpaper_original" -negate "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Negate effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Negate applied"
