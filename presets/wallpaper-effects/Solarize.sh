...
wallpaper_output="$cache_dir/solarize_${focused_monitor}.png"
...
magick "$wallpaper_original" -solarize 80% "$wallpaper_output"

if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Solarize effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Solarize applied"
