...
wallpaper_output="$cache_dir/frame_sunk_${focused_monitor}.png"
...
# This prevents FrameSunk -> FrameRaised from stacking effects.
...
if [[ ! -f "$wallpaper_output" ]]; then
    notify-send -u normal -i "$IDIR/error.png" "Wallpaper EffectZ" "Failed to create Frame Sunk effect"
    exit 1
fi
...
notify-send -u low -i "$IDIR/ja.png" "Wallpaper EffectZ" "Frame Sunk applied"
